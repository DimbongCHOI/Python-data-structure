...
def checkAllParentheses(val):
    checkStack = Stack(len(val))
    pair = {')':'(', '}':'{', ']':'['} # 짝 생성 : 닫는 괄호 -> 여는 괄호

    for ch in val:
        if(ch in '({['):
            checkStack.push(ch)
        elif(ch in ')}]'):
            if(checkStack.isStackEmpty()): # 여는 괄호 없이 닫힌 괄호
                return False # False
            checkPair = checkStack.pop() # 여는 괄호 pop
            if(checkPair != pair[ch]): # 여는 괄호의 닫는 괄호 짝이 안 맞으면
                return False # False
